    ...
    def _muon_step(self, stacked_grads, stacked_params, momentum_buf, v_buf, shape, red_dim):
        ...
        if shape[-2] > shape[-1]:
            for a, b, c in _PR_POLAR_EXPRESS_COEFFS[:self.ns_steps]:
                ...
                B = mx.addmm(A, A, A, alpha=c, beta=b)    # c*(A@A) + b*A, fused
                X = mx.addmm(X, X, B, alpha=1.0, beta=a)  # a*X + X@B, fused
        else:
            for a, b, c in _PR_POLAR_EXPRESS_COEFFS[:self.ns_steps]:
                A = X @ mx.swapaxes(X, -1, -2)
                B = mx.addmm(A, A, A, alpha=c, beta=b)    # c*(A@A) + b*A, fused
                X = mx.addmm(X, B, X, alpha=1.0, beta=a)  # a*X + B@X, fused
...
USE_MUON = True
NS_STEPS = 3  # Polar-Express iterations (was 5); fewer iters -> faster Muon step -> more total steps
MUON_MOMENTUM = 0.85
MUON_BETA2 = 0.97425
...
_loss_grad_fn = nn.value_and_grad(model, lambda model, inputs, targets: model(inputs, targets=targets))

# Compile forward+loss+grad into one fused graph. inputs/outputs=model.state so the
# eager optimizer's per-step param updates don't trigger recompilation; the optimizer
# step stays eager (handles the per-step LR/momentum/wd scalars without recompiles).
@partial(mx.compile, inputs=model.state, outputs=model.state)
def loss_grad_fn(inputs, targets):
    return _loss_grad_fn(model, inputs, targets)
...
while True:
    ...
    # Point 3: one eval per step -- microbatch grads accumulate lazily and get
    # materialized once (here on the first step for compile timing, otherwise
    # together with the optimizer update below), instead of an eval per microbatch.
